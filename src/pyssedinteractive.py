...
# Importing extra modules needed for interactive HR diagram plotting add-on
...
# -----------------------------------------------------------------------------
# Creating .html file for offline interactive H-R diagram with tooltips
# -----------------------------------------------------------------------------
...
def filter_and_convert(df, filter_cols, numeric_cols):
    ...
    for col in filter_cols:
        df[col] = pd.to_numeric(df[col], errors='coerce')  # '-', '[]', etc. → NaN
        df = df[df[col].notna()]                            # drop NaN
        df = df[df[col] != 0]                               # drop zeros
...
# --------------------------------------------------------------------
# Split by Evolution Stage [CURRENTLY UNUSED]
# --------------------------------------------------------------------
...
# --------------------------------------------------------------------
# HTML EXPORT  — injects a pure-JS image tooltip into the saved file
# --------------------------------------------------------------------
TOOLTIP_JS = """
<style>
  #star-tooltip {
  ...
  #star-tooltip .tt-name  { color: #79c0ff; font-weight: 600; font-size: 12px; margin: 2px 0; }
  #star-tooltip .tt-stats { color: #8b949e; font-size: 11px; margin: 2px 0; }
  #star-tooltip .tt-val   { color: #e6edf3; }
</style>
 
<div id="star-tooltip">
  <img id="tt-img" src="" alt="Star SED" />
  <p class="tt-name" id="tt-name"></p>
  <p class="tt-stats">T = <span class="tt-val" id="tt-teff"></span> K</p>
  <p class="tt-stats">L = <span class="tt-val" id="tt-lum"></span> L&#9737;</p>
  <p class="tt-stats">d = <span class="tt-val" id="tt-dist"></span> pc</p>
</div>
 
<script>
(function() {
  var tt      = document.getElementById('star-tooltip');
  var ttImg   = document.getElementById('tt-img');
  var ttName  = document.getElementById('tt-name');
  var ttTeff  = document.getElementById('tt-teff');
  var ttLum   = document.getElementById('tt-lum');
  var ttDist  = document.getElementById('tt-dist');
 
  // Wait for Plotly to be ready then attach listeners
  function attachListeners() {
    var graphs = document.querySelectorAll('.js-plotly-plot');
    if (!graphs.length) { setTimeout(attachListeners, 300); return; }
 
    graphs.forEach(function(gd) {
 
      gd.on('plotly_hover', function(eventData) {
        var pt = eventData.points[0];
        var cd = pt.customdata;          // [Object, Teff, Lum, Dist, IMG_URL]
        if (!cd) return;
 
        var imgSrc = cd[4];
        if (imgSrc && imgSrc !== 'None' && imgSrc !== 'null') {
          ttImg.src = imgSrc;
        ...
        ttName.textContent  = 'Gaia DR3 ' + cd[0];
        ttTeff.textContent  = parseFloat(cd[1]).toFixed(0);
        ttLum.textContent   = parseFloat(cd[2]).toFixed(4);
        ttDist.textContent  = parseFloat(cd[3]).toFixed(0);
...
def save_html_with_tooltips(fig, filepath):
    """
    Save a Plotly figure to a fully self-contained HTML file with
    image-rich hover tooltips powered by injected JavaScript.
 
    Requirements: customdata columns must be ordered as
        [Object, Teff, Luminosity, Distance, IMG_URL]
    which matches the custom_data= argument in make_hr_diagram().
    """
    # Write the base HTML (CDN Plotly.js to keep file size manageable;
    # swap include_plotlyjs=True for a fully offline file at ~3 MB extra)
    pio.write_html(fig, file=filepath, auto_open=False, include_plotlyjs=True)
                   
    # Inject tooltip HTML+CSS+JS just before </body>
    with open(filepath, 'r', encoding='utf-8') as f:
        html_content = f.read()
 
    html_content = html_content.replace('</body>', TOOLTIP_JS + '\n</body>')
 
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(html_content)
 
    print(f'Saved interactive HTML with image tooltips → {filepath}')
...
#    if (proctype!="none"):
    ...
    # -------------------------------------------
    # Add-on for interactive .html H-R diagram. Uses Javascript for offline tooltips
    plothrinteractive=int(pyssedsetupdata[pyssedsetupdata[:,0]=="MakeHRInteractive",1][0])
    if plothrinteractive == 1:

        columns_mapping = {
            'Object': '#Object', 'RA': 'RA', 'Dec': 'Dec', 'Distance': 'Distance',
            'Teff': 'Teff', 'Luminosity': 'Lum', 'log(g)': 'logg', 'E(B-V)': 'E(B-V)'
        }
        df = load_and_clean_data('../output/output.dat', columns_mapping=columns_mapping)
        ...
        # Save standalone HTML with image tooltips --------------------
        save_html_with_tooltips(fig, '../output/Interactive_HR_Diagram.html')
